...
def GraSP(net, ratio, train_dataloader, device, num_classes=10, samples_per_class=25, num_iters=1, T=200, reinit=False, scaled_init=False):
    eps = 1e-10
    keep_ratio = 1-ratio
    old_net = net
            ...
            for idx, layer in enumerate(net.modules()):
                if isinstance(layer, nn.Linear):
                    ...
                    scaling = (layer.weight ** 2 * keep_masks[
                        old_modules[idx]] / sigma_w2 ** 2 * number_of_channels * filter_size ** 2).mean(dim=1)
                    scaling_vals = 1. / (torch.sqrt(scaling) + 0.001)
